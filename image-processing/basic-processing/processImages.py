...
def pintaImagen(imagen,textoGeneral,textoImagen):
    cv2.putText(imagen, textoImagen, (10,30), cv2.FONT_HERSHEY_SIMPLEX, 1,(255,255,255),2,cv2.LINE_AA)
...
print("¿Qué imagen quiere utilizar: Poca_iluminacion[1], Ejemplo_ruido[2], Ruido_pepper[3], Ruido_gaussiano[4]?")
tipo_img = int(input())
...
if tipo_img == 1:
    ...
    #img = cv2.imread('oscuro.png')
    print("Elegido poca iluminacion")
elif tipo_img == 2:
    img = cv2.imread('Image_noise-example.jpg')
    print("Elegido noise example")
elif tipo_img == 3:
...
if tipo_proc == 1:
    ...
    cv2.imshow('Imagen con operación suma', added)
    cv2.imshow('Imagen con threshold binario inverso', cv2.bitwise_and(gray_image,gray_image,mask = thresh3))
    cv2.imshow('Imagen con threshold adaptativo',thresh1)
    cv2.imshow('Imagen con threshold adaptativo gaussiano',thresh2)
    cv2.imshow('Imagen negativa', negative_image)
    cv2.imshow('Imagen con power law transformation', power_law)
    cv2.waitKey(0)
elif tipo_proc == 3:
    gray_image = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    ...
    cv2.imshow("Imagen con ecualizacion", equ)
    cv2.imshow('Imagen con CLAHE', cl1)
elif tipo_proc == 4:
    ...
    cv2.imshow('Imagen con convolucion 2D aplicando operaciones de vecinos', dst)
    cv2.imshow('Imagen con blur', blur)
    cv2.imshow('Imagen con gaussian blur', gaussianblur)
    cv2.imshow('Imagen con filtro para salt-and-pepper noise', median)
    cv2.imshow('Imagen con filtro para noise removal while keeping edges sharp', bilateralFilter)
